 ...
 *
 * @method \App\Model\Entity\ExamType[] paginate($object = null, array $settings = [])
 */
class ExamTypesController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->ExamTypes->save($examType)) {
                $this->Flash->success(__('The {0} has been saved.', 'Exam Type'));
                return $this->redirect(['action' => 'index']);
            } else {
                $this->Flash->error(__('The {0} could not be saved. Please, try again.', 'Exam Type'));
            }
        }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->ExamTypes->delete($examType)) {
            $this->Flash->success(__('The {0} has been deleted.', 'Exam Type'));
        } else {
            $this->Flash->error(__('The {0} could not be deleted. Please, try again.', 'Exam Type'));
        }
        return $this->redirect(['action' => 'index']);
    }
}
